...
// Проверка авторизации (только админы могут добавлять платежи)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $payer_id = filter_input(INPUT_POST, 'payer_id', FILTER_VALIDATE_INT);
    $amount = filter_input(INPUT_POST, 'amount', FILTER_VALIDATE_FLOAT);
    $payment_date_str = filter_input(INPUT_POST, 'payment_date', FILTER_SANITIZE_STRING);
    $description = filter_input(INPUT_POST, 'description', FILTER_SANITIZE_STRING);
    $status = filter_input(INPUT_POST, 'status', FILTER_SANITIZE_STRING);

    // Валидация данных
    if ($payer_id === false || $payer_id <= 0) {
    ...
    if (empty($payment_date_str)) {
        ...
        if (!$payment_date) {
            ...
            $payment_date = $payment_date->format('Y-m-d H:i:s'); // Format for database
        }
    }

    if (empty($status)) {
        $errors[] = "Выберите статус платежа.";
    }

    // Если нет ошибок, добавляем платеж в базу данных
    if (empty($errors) && isset($payment_date)) {
        try {
            // Assuming 'id' is the primary key for residents
            $stmt = $pdo->prepare("INSERT INTO payments (tsj_id, resident_id, amount, payment_date, description, status) VALUES (:tsj_id, :resident_id, :amount, :payment_date, :description, :status)");
            $stmt->execute([
                'tsj_id' => $id_tszh,
                'resident_id' => $payer_id,
                'amount' => $amount,
...
// Получение списка жильцов для выбора плательщика
$residents = [];
try {
    // Selecting from the residents table
    $stmt_residents = $pdo->prepare("SELECT id, full_name FROM residents WHERE tsj_id = :id_tszh ORDER BY full_name");
    $stmt_residents->execute(['id_tszh' => $id_tszh]);
    $residents = $stmt_residents->fetchAll(PDO::FETCH_KEY_PAIR | PDO::FETCH_COLUMN, 1); // Keyed by id, value is full_name
} catch (PDOException $e) {
        ...
        <div class="form-container">
            ...
            <form method="POST" action="">
                <div class="form-group">
                    <label for="payer_id">Плательщик (Жилец):</label>
